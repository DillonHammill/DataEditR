...
**DataEditR** ships with three distinct shiny modules, namely `dataInput`, `dataEdit` and `dataOutput` which have been wrapped up into a single function called `data_edit()` to make it possible to interactively view, enter or edit data in R. You can use `data_edit()` as a standalone application, or include the relevant modules within your own shiny applications. Alternatively,
you can use the RStudio add-in that ships with `DataEditR`. 
...
- code required to create edited data can be optionally printed to the console
